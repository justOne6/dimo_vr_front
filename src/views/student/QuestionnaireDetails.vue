<template>
  <div class="page-restrict-width questionnaire-wrapper">
    <p v-if="!isAlreadyTreated">
      ...
      Les mauvaises réponses ne vous enlèvent pas de points.<br />
      Vous ne pouvez donc pas obtenir un score négatif et vous aurez un score de
      0 si vous n'avez aucune bonne réponse.<br />
      Votre résultat sera affiché à la fin du questionnaire en pourcentage.
    </p>
    <h5 class="resultat" v-if="isAlreadyTreated">
      Vous avez obtenu un score de {{ this.score }}% à ce questionnaire.
    </h5>
    <v-card>
      ...
      <v-divider></v-divider>
      <v-btn v-if="!isAlreadyTreated" @click="saveAnswers" color="primary"
        >Envoyer les réponses</v-btn
      >
    </v-card>
  </div>
</template>
      ...
      this.questionnaire.questions.forEach((question) => {
        question.options.forEach((option) => {
          this.$set(
            option,
            "selected",
            data.answers.some((answer) => answer.option_id === option.id)
          );
        });
      });
    },
    ...
    async saveAnswers() {
      ...
      try {
        ...
        if (error.response.status === 400) {
          alert(
            "Vous ne pouvez pas répondre à ce questionnaire plusieurs fois."
          );
        } else {
